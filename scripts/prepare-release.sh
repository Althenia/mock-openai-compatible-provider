...
if [ -n "$sources" ]; then
  ...
  curl -fL --connect-timeout 15 --max-time 180 --retry 2 -o "$assets/bun-1.4.0-source.tar.gz" \
    https://codeload.github.com/oven-sh/bun/tar.gz/refs/tags/bun-v1.4.0
  ...
  bash scripts/archive-webkit.sh "$assets/webkit-0f966e81-source.tar.gz"
fi
