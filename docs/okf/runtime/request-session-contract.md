...
A recognized webchat safety block returns HTTP 422 with code `webchat_safety_block` before streaming. If reasoning has already streamed, the handler sends a terminal structured SSE error carrying that code instead of a successful completion. Responses history is not committed for the failed stream.[^server] The provider does not automatically retry or fall back after the block and records a definitive failed browser attempt.[^runtime-guide]
