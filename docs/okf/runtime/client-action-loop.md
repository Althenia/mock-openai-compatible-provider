...
Action envelopes carry the current submission turn key, a unique call ID, an offered name, and schema-conforming input; chat envelopes stay text, thinking envelopes become reasoning, and quoted action JSON inside chat is not executed.[^runtime-guide] Every initialization, task, result, repair, and correction submission carries its current `TURN KEY` and short every-turn envelope guard.[^runtime-guide] The protocol defines `<aipass-envelope>` delimiters, a leading `TURN KEY` line, per-turn key matching, and a typed action chain of thinking, action group, and final chat or action envelopes.[^protocol]
